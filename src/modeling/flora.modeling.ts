...
function makeTree(treeHeight: number, verticalSegments: number, radius: number, seed: number) {
  ...
  const largeTreeBase = new MoldableCubeGeometry(3, treeHeight, 3, 4, verticalSegments, 4)
    .cylindrify(radius)
    .translateMc(0, treeHeight / 2, 0);

  let scale = 1.0;
  doTimes(verticalSegments + 1, index => {
    const yPos = index * segmentSize;
    largeTreeBase.selectBy(vertex => vertex.y === yPos)
      .scaleMc(scale, 1, scale);
    if (index % 2 === 0) {
      scale *= 0.7;
    } else {
      largeTreeBase.translateMc(randomNumber(index + treeHeight + seed), 0, randomNumber(index + treeHeight + seed));
    }

    if (index === verticalSegments) {
      largeTreeBase.scaleMc(0, 1.2, 0);
    }
  });
  return largeTreeBase.doneMc();
}
...
function makeLeavesGeo(
  ...
  return new MoldableCubeGeometry(fidelity, fidelity, fidelity, fidelity, fidelity, fidelity)
    .spherify(radius)
    .translateMc(translateX, translateY, translateZ)
    .scaleMc(scaleX, scaleY, scaleZ)
...
export function makeLargeTreeGeo() {
  ...
  branch1.allMc().rotateMc(0, 0, 1).translateMc(0, 5, -0.2).doneMc();
  branch2.allMc().rotateMc(0.8, 0, -1).translateMc(0, 8, -0.2).doneMc();

  return treeBase.merge(branch1).merge(branch2).computeNormalsCrossPlane().doneMc();
}

export function makeTreeLeavesGeo() {
  const leavesGeo1 = makeLeavesGeo(3, 3, 2, -3, 8, -1, 2, 1, 1.7).doneMc();
  const leavesGeo2 = makeLeavesGeo(3, 3, 5, 2.5, 10, 3, 2, 1, 1.8).doneMc();
  const leavesGeo3 = makeLeavesGeo(3, 4, 7, 0, 9.3, 0, 2, 1.5, 1.9).doneMc();

  return leavesGeo1.merge(leavesGeo2).merge(leavesGeo3).doneMc();
}
...
function makePlantLeaves(leafScaleX: number, leafScaleY: number, leafScaleZ: number) {
  return new MoldableCubeGeometry(2, 2, 1, 2, 1)
    ...
    .scaleMc(leafScaleX, leafScaleY, leafScaleZ)
    .selectBy(vertex => vertex.y > 0 && vertex.x === 0)
    .translateMc(0, -2, 0)
    .doneMc();
}
...
export function makePlantGeo() {
  ...
  return bush1Geo.merge(makePlantLeaves(1, 3, 0).allMc().rotateMc(0, 0.8)).allMc().translateMc(0, 1).doneMc();
}
